document.addEventListener('DOMContentLoaded', function () {
    const BASE_URL = window.location.hostname.includes("localhost") || window.location.hostname.includes("127.")
        ...
        // Obtener valores de un select multiple controlado por Choices.js
    ...
    document.getElementById("candidateForm").addEventListener("submit", async function (event) {
        ...
        if (!is18OrOlder) {
            ...
            alert("⚠️ Debes tener al menos 18 años para enviar este formulario.")
            return;
        }
        ...
        if (!formData.get("gender")) {
            ...
            alert("⚠️ Debes seleccionar un género.");
            isValid = false;
        }

        if (!formData.get("birthDate")) {
            showError("birthDate", "Debe ingresar una fecha de nacimiento.");
            alert("⚠️ Debes ingresar una fecha de nacimiento.");
            isValid = false;
        }

        if (!formData.get("educationLevel")) {
            showError("educationLevel", "Debe seleccionar un nivel educativo.");
            alert("⚠️ Debes seleccionar un nivel educativo.");
            isValid = false;
        }

        const technicalSkills = getSelectedValues("technicalSkills");
        if (technicalSkills.length === 0) {
            showError("technicalSkills", "Debe seleccionar al menos una habilidad tecnica.");
            alert("⚠️ Debes seleccionar al menos una habilidad tecnica.");
            isValid = false;
        }
        ...
        if (isNaN(parseFloat(formData.get("interviewScore"))) ||
            ...
            alert("⚠️ Ingrese una puntuación válida para la entrevista (0-10).");
            isValid = false;
        }
        ...
        if (isNaN(parseInt(formData.get("experience"))) || parseInt(formData.get("experience")) < 0) {
            ...
            alert("⚠️ Ingrese una experiencia válida en años.");
            isValid = false;
        }
        ...
        if (isNaN(parseInt(formData.get("icfes"))) || parseInt(formData.get("icfes")) < 0 || parseInt(formData.get("icfes")) > 500) {
            ...
            alert("⚠️ Ingrese un puntaje de ICFES válido (0-500).");
            isValid = false;
        }

        const softSkills = getSelectedValues("softSkills");
        if (softSkills.length === 0) {
            showError("softSkills", "Debe seleccionar al menos una habilidad blanda.");
            alert("⚠️ Debes seleccionar al menos una habilidad blanda.");
            isValid = false;
        }


        const languages = getSelectedValues("languages");
        if (languages.length === 0) {
            showError("languages", "Debe seleccionar al menos un idioma.");
            alert("⚠️ Debes seleccionar al menos un idioma.");
            isValid = false;
        }
        ...
        if (!formData.get("workPreference")) {
            ...
            alert("⚠️ Debes seleccionar una preferencia de trabajo.");
            isValid = false;
        }

        const personalStrengths = getSelectedValues("personalStrengths");
        if (personalStrengths.length === 0) {
            showError("personalStrengths", "Debe seleccionar al menos una fortaleza personal.");
            alert("⚠️ Debes seleccionar al menos una fortaleza personal.");
            isValid = false;
        }
        ...
        try {
            ...
            if (verificarData.emailExiste || verificarData.telefonoExiste) {
                alert("⚠️ El numero de telefono o el correo electronico que diligenciaste ya fueron ingresados en otro registro.");
                isValid = false;
            }
        ...
        if (isValid) {
            ...
            data.personalStrengths = personalStrengths;
...
//             alert("⚠️ Debes seleccionar al menos una habilidad técnica.");
//             isValid = false;
//         }
